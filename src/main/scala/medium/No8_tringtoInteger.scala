...
object No8_tringtoInteger {
  ...
  def myAtoi(str: String): Int = {
    ...
    var result = 0l
    for (c <- newStr) {
      if (!c.isDigit) { //如果遇到不是数字类型的，直接返回
        return result.toInt * isInf
      } else { //否则进行数据进位加和
        result = result * 10 + c.toInt - 48
        //判断数据是否超过边界了
        if (isInf == -1 && (-result) <= Int.MinValue) return Int.MinValue
        if (isInf == 1 && result >= Int.MaxValue) return Int.MaxValue
      }
    }
    result.toInt * isInf
  }
